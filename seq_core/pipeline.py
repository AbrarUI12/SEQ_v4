...
def resolve_torch_dtype(dtype_arg: str, device: str = "cuda") -> torch.dtype:
    return resolve_dtype(dtype_arg, device)
...
def resolve_experiment_config(config: Dict[str, Any], exp: Dict[str, Any]) -> Dict[str, Any]:
    resolved = {
        ...
        "seq_variant": config.get("seq_variant", {}),
    }
...
def run_experiment(
    ...
    if resolved.get("seq_variant"):
        with (run_dir / "variant_config.yaml").open("w") as f:
            yaml.safe_dump(resolved["seq_variant"], f, sort_keys=False)
    ...
    if act_path.exists():
        ...
        act_table = collect_activation_entropy(
            ...
            degeneracy_mode=degeneracy_mode,
            activation_masking=bool(resolved.get("seq_variant", {}).get("activation_masking", False)),
        )
    ...
    if policy_type == "risk_score":
        for name, module in model.named_modules():
            if not isinstance(module, torch.nn.Linear):
                continue
            if weight_table.get(name, {}).get("entropy_bits") is None:
                unreliable_modules.add(name)
            if act_table.get(name, {}).get("entropy_bits") is None:
                unreliable_modules.add(name)
    ...
    if policy_type == "risk_score":
        variant_cfg = resolved.get("seq_variant", {})
        base_map, risk_info = assign_risk_score_tiers(
            weight_table,
            act_table,
            unreliable_modules,
            fp16_threshold=float(variant_cfg.get("risk_fp16_threshold", 0.90)),
            int8_threshold=float(variant_cfg.get("risk_int8_threshold", 0.65)),
        )
    else:
    ...
    quant_cfg = resolved.get("quantization") or {}
    bnb_compute_dtype = resolve_torch_dtype(str(quant_cfg.get("bnb_compute_dtype", "float16")), device)
    quant_info = apply_mixed_precision(
        model,
        final_map,
        device=device,
        dtype_fp16=dtype,
        bnb_4bit=str(quant_cfg.get("bnb_4bit_quant_type", "nf4")),
        bnb_compute_dtype=bnb_compute_dtype,
        double_quant=bool(quant_cfg.get("bnb_4bit_use_double_quant", True)),
    )
