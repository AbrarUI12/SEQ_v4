...
def collect_activation_entropy(
    ...
    hook_context: Dict[str, Optional[torch.Tensor]] = {"attention_mask": None}

    def _select_real_tokens(output: torch.Tensor) -> torch.Tensor:
        if not activation_masking:
            return output
        attention_mask = hook_context.get("attention_mask")
        if attention_mask is None:
            return output
        try:
            mask = attention_mask.to(output.device).bool()
            if (
                output.dim() == 3
                and mask.dim() == 2
                and mask.shape[0] == output.shape[0]
                and mask.shape[1] == output.shape[1]
            ):
                return output[mask]
        except Exception:
            return output
    ...
    def make_hook_pass1(name: str):
        def hook(_module, _inputs, output):
            if isinstance(output, tuple):
                output = output[0]
            values = _select_real_tokens(output).detach()
    ...
    with torch.no_grad():
        for prompt in prompts:
            ...
            hook_context["attention_mask"] = inputs.get("attention_mask")
            _ = model(**inputs)
            hook_context["attention_mask"] = None
    ...
    def make_hook_pass2(name: str):
        def hook(_module, _inputs, output):
            ...
            mean = summary["mean"]
            std = summary["std"]
            values = _select_real_tokens(output).detach().to(dtype=torch.float32)
            flat = values.reshape(-1)
            mask = torch.isfinite(flat)
            flat = flat[mask]
            if flat.numel() == 0:
