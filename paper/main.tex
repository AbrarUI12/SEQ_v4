...
%
% Compiles out of the box with a plain two-column fallback. To use the official ACL style,
% drop `acl.sty` and `acl_natbib.bst` (from https://github.com/acl-org/acl-style-files)
% into this directory and recompile: the \IfFileExists switch below picks them up.
%
...
\bottomrule
\end{tabular}
\caption{Llama-3.2-3B operating points. Accuracy is \texttt{acc\_norm} where the task provides
it and \texttt{acc} otherwise, macro-averaged over six tasks, from the same artifact the
confidence intervals are computed from.}
\label{tab:operating}
\end{table}
...
\texttt{act\_max} 8.108, \texttt{residual\_rms} 8.141 vs \texttt{random} 8.319). Downstream this
